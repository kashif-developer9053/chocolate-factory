...
import { Eye, Search, Download, Package, Truck, CheckCircle, XCircle, Mail } from "lucide-react";
...
export default function AdminOrdersPage() {
  ...
  const [emailPopupDetails, setEmailPopupDetails] = useState({ orderId: "", email: "", status: "" });
  ...
  useEffect(() => {
    if (showEmailPopup) {
      const timer = setTimeout(() => setShowEmailPopup(false), 3000);
  ...
  const fetchOrders = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        const newStats = formattedOrders.reduce(
          (acc, order) => {
            acc.total++;
        ...
        setStats(newStats);
        setMessage(`Loaded ${formattedOrders.length} orders successfully`);
  ...
  const updateOrderStatus = async (orderId, newStatus) => {
    try {
      ...
      console.log("Updating order:", { orderId, newStatus, order });
      ...
      if (response.data.success) {
        ...
        try {
          ...
          const emailResponse = await axios.post("/api/admin/orders/notify", {
            orderId,
            newStatus,
            customerEmail: order.customer.email,
          });
          if (emailResponse.data.success) {
            ...
            setMessage(`Failed to send email: ${emailResponse.data.message}`);
          }
        } catch (emailError) {
          console.error("Error sending email:", emailError);
          setMessage("Failed to send notification email");
        }
  ...
  const exportOrders = () => {
    const csvContent =
      ...
      filteredOrders
        .map(
          (order) =>
            `${order._id.slice(-6)},${order.customer.firstName} ${
              order.customer.lastName
            },${order.customer.email},${order.customer.phone},${order.pricing.total},${
              order.orderStatus
            },${formatDate(order.orderDate)},${order.trackingNumber || "N/A"}`
        )
        .join("\n");
  ...
  return (
    <>
      <Head>
        <title>Admin Orders - The Chocolate Factory</title>
              ...
              <div className="p-6">
                <div className="flex justify-between items-center mb-6 border-b border-gray-200 pb-4">
                  ...
                  <button
                    ...
                    className="text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-lg px-4 py-2 transition-colors"
                  >
                    ✕ Close
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  <div className="bg-gray-50 rounded-lg p-4">
                    ...
                    <div className="space-y-2 text-sm">
                      <p>
                        <span className="font-medium">Name:</span> {selectedOrder.customer.firstName}{" "}
                      ...
                      <p>
                        <span className="font-medium">Username:</span> {selectedOrder.customer.username}
                      </p>
                    </div>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-4">
                    ...
                    {selectedOrder.items.map((item, index) => (
                      <div key={index} className="flex items-center gap-4 p-4">
                        <img
                          src={
                            item.image ||
                            `https://via.placeholder.com/60?text=${encodeURIComponent(item.name)}`
                          }
                          alt={item.name}
          ...
          <div className="fixed inset-0 bg-black bg-opacity-30 flex justify-center items-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-lg p-6 max-w-sm w-full animate-fade-in">
              ...
              <p className="text-sm text-gray-600 mb-4">
                Notification for Order #{emailPopupDetails.orderId} (Status:{" "}
                {emailPopupDetails.status}) sent to{" "}
                <span className="font-medium">{emailPopupDetails.email}</span>.
              </p>
              <button
